...
<Dialog.Root>
	...
	<Dialog.Content class="max-w-[700px] w-full backdrop-blur-0 bg-white">
		...
		<div class="flex gap-5">
			{#each data.contents.filtersGroups as filtersGroup}
				<div class="w-full">
					<h1 class="font-medium text-xs uppercase py-5 border-b">{filtersGroup.title}</h1>
					<div class="mt-5 space-y-5 flex flex-col">
						{#each data.contents.filterGroup.filters as filter}
							<button
								on:click={() => {
								...
								class="font-medium text-xs captalize"
							>
								{filter.label}
							</button>
						{/each}
